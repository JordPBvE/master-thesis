...
class Holstmark(Distribution):
    def __init__(self):
        self.f = lambda x : stats.levy_stable.pdf(x, alpha=1.5, beta=0, loc=0, scale=1)
        self.f_hat = lambda w : np.exp(-np.abs(w)**1.5)
...
def precompute_shannon_coeffs(dist, m, k_max):
    ...
    for i, k in enumerate(ks):
        ...
        T, _ = integrate.quad(func, -bound, bound)
        cmks[i] = factor * T
    return cmks
...
def precompute_meyer_coeffs(dist, m, k_max, nu_func):
    ...
    for i, k in enumerate(ks):
        ...
        T2, _ = integrate.quad(func_T2, 0, a)
        T3, _ = integrate.quad(func_T3, a, b)
...
def get_shannon_projection(cmks_full, m, K, ts):
    """Slices cmks and projects using sinc matrix"""
    k_max = (len(cmks_full) - 1) // 2
    cmks = cmks_full[k_max - K : k_max + K + 1]
    
    ks = np.arange(-K, K + 1)
    args = (2**m * ts[:, np.newaxis]) - ks[np.newaxis, :]
    phi_matrix = 2**(m/2) * np.sinc(args)
    
    return np.dot(phi_matrix, cmks)

def get_meyer_projection(cmks_full, m, K, nu_func, ts, ws):
    """Slices cmks, reconstructs in frequency, then transforms to time."""
    k_max = (len(cmks_full) - 1) // 2
    cmks = cmks_full[k_max - K : k_max + K + 1]
    ks = np.arange(-K, K + 1)
    ...
    time_domain = [np.real(integrate.simpson(1/(2*np.pi) * freq_domain * np.exp(1j * ws * t), ws)) for t in ts]
...
densities = [
    Gaussian(0, 1), 
    NormalInverseGaussian(a=0.3, b=0.2, loc=0.0, scale=1.0)
]

fig, ax = plt.subplots(len(densities), len(ms) + 1, figsize=(15, 10))

for i, density in enumerate(densities):
    analytic = density.f(xs)
    ax[i, 0].plot(xs, analytic, '-', lw=2)
    ax[i, 0].set_title(f'{type(density).__name__} Analytic')
    if i == (len(densities) - 1) : ax[i, 0].set_xlabel("x")
    ax[i, 0].grid(True, alpha=0.1)

    for j, m in enumerate(ms):
        k_max_limit = 24 * 2**m
        Ks_to_plot = np.linspace(0, k_max_limit, 50, dtype=int)
        
        ws = np.linspace(-2**(m+2) * np.pi / 3, 2**(m+2) * np.pi / 3, 400)
        ...
        for K in Ks_to_plot:
            proj_s = get_shannon_projection(coeffs_shannon, m, K, xs)
            errs_s.append(np.linalg.norm(analytic - proj_s, 2))
            
            proj_mx = get_meyer_projection(coeffs_meyer_x, m, K, nu_lin, xs, ws)
            errs_mx.append(np.linalg.norm(analytic - proj_mx, 2))
            
            proj_mp = get_meyer_projection(coeffs_meyer_p, m, K, nu_poly, xs, ws)
            errs_mp.append(np.linalg.norm(analytic - proj_mp, 2))

        ax[i, j+1].plot(Ks_to_plot, errs_s, '-', label='Shannon', alpha=0.7)
        ax[i, j+1].plot(Ks_to_plot, errs_mx, '-', label='Meyer Lin', alpha=0.7)
        ax[i, j+1].plot(Ks_to_plot, errs_mp, '-', label='Meyer Poly', alpha=0.7)
        
        ax[i, j+1].set_title(f"m = {m}")
        if i == (len(densities) - 1) : ax[i, j+1].set_xlabel("K")
        if j==0: ax[i, j+1].set_ylabel("L2 Error")
        ax[i, j+1].set_yscale('log')
        ax[i, j+1].grid(True, alpha=0.1)
        if i==0 and j==(len(ms)-1): ax[i, j+1].legend(fontsize='small')
