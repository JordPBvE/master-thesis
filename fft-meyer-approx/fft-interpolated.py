...
class Gaussian(Distribution):
    def __init__(self, mu, sigma):
        super().__init__(
            ...
            lambda ws : np.exp(- mu * 1j * ws - 0.5 * sigma * np.power(ws, 2))
        )
...
def I_lin(k, coeffs):
    ...
    a = 1j * 2 * np.pi / 3
    t1 = (p1 * np.exp(a*k_safe) - p0) / (a*k_safe)
    t2 = (p1-p0) * (np.exp(a*k_safe) - 1) / (a**2 * np.power(k_safe,2))
    safe_result = t1 - t2

    return np.where(k == 0, (p0 + p1) / 2, safe_result)
...
def meyer_T3s(distr : Distribution, nu, m, N):
    ...
    return ifft_input, np.array([p0, 0])
...
def meyer_T2s(distr : Distribution, m, N):
    ...
    return ifft_input, np.array([p0, p1])
...
def meyer_coefficients(distr, nu, m, N):
    ...
    T2s = factor_2 * np.real(T2_ifft + I_lin(ks, T2_coeffs) / 3)
    T3s = factor_3 * (T3_ifft + I_lin(ks, T3_coeffs) / 3)
